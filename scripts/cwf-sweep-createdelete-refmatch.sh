...
# CWF create-delete sweep — REFERENCE-MATCHED (run_benchmark_fullsystem.sh) EXCEPT sampling.
#   Same per-point cube-bench cmd; sampling kept as ours (n = c*10, single depth).
#   Matched to reference: cleanup BEFORE run, then run, then sleep 30, then
#   drop_caches AFTER run WITHOUT sync (reference order/no-sync).
...
for c in $(seq 100 10 350); do
  n=$((c*10))
  cleanup                                   # reference: cleanup BEFORE the run
  ...
  sleep 30                                  # reference: settle 30s after run
  echo 3 > /proc/sys/vm/drop_caches || true # reference: drop AFTER run, no sync
done
