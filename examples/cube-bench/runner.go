...
func benchOne(client *http.Client, cfg *Config, seq int) IterResult {
	...
	if cfg.Mode == "create-delete" && cr.SandboxID != "" {
		...
		dresp.Body.Close()
		// A sandbox deleted immediately after create can still be restoring,
		// which the API reports as a transient 5xx; retry until it drains.
		for attempt := 0; attempt < 30 && dresp.StatusCode != 200 && dresp.StatusCode != 204; attempt++ {
			time.Sleep(200 * time.Millisecond)
			rreq, rerr := http.NewRequest("DELETE", apiURL+"/sandboxes/"+cr.SandboxID, nil)
			if rerr != nil {
				r.Err = fmt.Sprintf("delete request build: %v", rerr)
				return r
			}
			for k, v := range headers {
				rreq.Header.Set(k, v)
			}
			dresp, err = client.Do(rreq)
			r.DeleteMs = float64(time.Since(t0).Microseconds()) / 1000.0
			if err != nil {
				r.Err = fmt.Sprintf("delete: %v", err)
				return r
