...
extern struct _hash_data* init_hash(uint32 max_cha);
extern int get_cha_id(struct _hash_data*hd, unsigned long long adr);
extern void load_pa(struct _hash_data*hd, Addr64 a);
extern unsigned int compare_and_pick(struct _hash_data *hd, int base, int limit, int n);
extern unsigned int cbo_id(struct _hash_data *hd, int n);
extern Addr64 H_bit (Addr64 p, uint32 i_bit);
extern Addr64 P_bit (Addr64 p, uint32 i_bit);
extern uint32 pmod3 (Addr64 hashed_addr);
extern uint32 pmod5 (Addr64 hashed_addr);
//*******//
...
unsigned int get_processor_signature(void);
unsigned short get_processor_model_id(void);
int get_cluster_id (unsigned long long pa, int num_clusters);

int is_sapphire_rapids(void);
int is_emerald_rapids(void);
int is_granite_rapids(void);
int is_diamond_rapids(void);
int is_sierra_forest(void);
int is_skylake(void);
int is_icelake(void);
int hash_mode=-1;

int get_hash_mode(void)
{
...
void my_cpuid(unsigned int  eax_in, unsigned int ecx_in, cpuid_info_t* info)
{
	unsigned int eax = 0;
	unsigned int ebx = 0;
	unsigned int ecx = 0;
...
int BindToCpu(int cpu_num)
{
	...
	status = sched_setaffinity(0, sizeof(cs), (my_cpu_set_t*)&cs);
...
unsigned long long get_physaddr(void *vaddr)
{
    ...
    //     addr &= (1ULL<<54)-1;
    //     addr <<= 12;
        return addr + ( ((unsigned long long)vaddr << 12)  & (4096-1));
}
