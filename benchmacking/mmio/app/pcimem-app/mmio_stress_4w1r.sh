#!/bin/bash
### tool to batch run the pcimem MMIO test with 4 MMIO write ops + 1 MMIO read ops , and loop cycle. 
...
if [ $scaling_enabled == 0 ];then
	...
	do 
		...
		taskset -c $core ./pcimem -m memio -b $base_addr -s 0x1000 -o m -r 0x20 -v 0x55AA -c 4 -w 0 -M 1 -l $loop_count > ${LOG_PATH}/pcimem-test-$(date +"%y%m%d-%H-%M-%S")-$i.log &
	done
	...
	do 
		...
		do 
			...
			if [ $i == $t_count ]; then
				...
				taskset -c $core ./pcimem -m memio -b $base_addr -s 0x1000 -o m -r 0x20 -v 0x55AA -c 4 -w 0 -M 1 -l $loop_count > ${T_LOG_PATH}/pcimem-test-$(date +"%y%m%d-%H-%M-%S")-$i.log
			else 
				taskset -c $core ./pcimem -m memio -b $base_addr -s 0x1000 -o m -r 0x20 -v 0x55AA -c 4 -w 0 -M 1 -l $loop_count > ${T_LOG_PATH}/pcimem-test-$(date +"%y%m%d-%H-%M-%S")-$i.log &
			fi 
		done
