app: mmio_stress for customer 
If memory access strict config is enabled in your system kernel, you can take one of options to disable the strict: 1, add kernell parameter in boot cmdline `iomem=relaxed`; OR 2,build kernel by disable `CONFIG_IO_STRICT_DEVMEM`.
...
```
./pcimem -h
Usage: ./pcimem -b <bar_base_hex> -s <bar_size_hex> -o <r|w> -r <reg_offset_hex> [-v <value>] [other options]
Options: (V1.1c)
  -b <bar_base>       Specify BDF for PCI or BAR base address for MMIO. 
...
and do read operation on register offset 0x20, will loop the operation 10 times. 
```
./pcimem -m memio -b 0x213ffc080000 -s 0x1000 -o r -r 0x20 -l 10
```
