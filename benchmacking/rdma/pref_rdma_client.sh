...
ib_write_bw_tool="/home/mz/tools/perftest_pref/ib_write_bw"
...
#test_opts="-l ${post_list_size} -s ${io_size} -t ${tx_depth} -D ${duration_report} --use_old_post_send --use_hugepages --disable_pcie_relaxed"
test_opts="-l ${post_list_size} -s ${io_size} -t ${tx_depth} -D ${duration_report} --use_hugepages --disable_pcie_relaxed"
if [ "$old_post" == "1" ]; then
	test_opts="$test_opts --use_old_post_send" 
fi
