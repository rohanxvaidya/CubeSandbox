...
outputfile="${data_path}/${TEST_OPERATION}_kpi_${client_n}client_${round}round_${title}.csv"
...
if [[ "${TEST_OPERATION}" =~ "random" || "${TEST_OPERATION}" =~ "sequential" ]]; then
        ...
        #         line=1
        #         test_cycle=test_cycle+1 
        #         # set end flag after noDSA, withDAS case finished current cycle(test_cycle).
        #         round_end=1  
        #     }
