I’m going to inspect the full source under the project’s src directory first, then trace argument parsing and every call path hit by your exact command so we can build a precise function-by-function flow. After static tracing, I’ll run the command on this system and correlate runtime behavior with the code paths.
