...
void print_usage(const char *prog_name) {
    fprintf(stderr, "Usage: %s -m <pci|memio> -b <bdf|bar_base_hex> -s <bar_size_hex> -o <r|w> -r <reg_offset_hex> [-v <value>] [other options]\n", prog_name);
    fprintf(stderr, "Options: (V%s)\n", PM_VERSION);
    fprintf(stderr, "  -m <pci|memio>      Specify the mode of operation.\n");
    fprintf(stderr, "  -b <bdf|bar_base>   Specify BDF for PCI or BAR base address for MMIO. (e.g.3e:00.0 for pci mode or 0x80000000 for mmio)\n");
    ...
    fprintf(stderr, "  -w <0|1|2|3|4>      Add certain instruction after MMIO write, by default is serialize (1), \n\
...
int main(int argc, char *argv[]) 
{
    ...
    char operation = 0;
    off_t reg_offset = 0;
    uint32_t loop_count = 1, value = 0, post_flag = POST_SERIALIZE, message_mode = 2, write_count = 4;
    ...
    int opt;
    while ((opt = getopt(argc, argv, "m:b:s:o:r:v:l:w:M:c:h")) != -1) {
        switch (opt) {
            case 'm':
                mode = optarg;
            ...
            case 'l':
                ...
                break;
            case 'w':
                post_flag = strtoul(optarg, NULL, 0);
                if (post_flag >= POST_LAST || post_flag < 0 ) {
                    fprintf(stderr, "Bad Instruction! change to the default instruction serialize \n");
                    post_flag = POST_SERIALIZE;     // by default is serialize instruction after write.
                }
                break;
            case 'M':
...
// Function execution
    ...
    if ((operation == 'w' && write_value_set == 1)) {
        for(int i = 0; i < loop_count; i++){
            ...
            if (message_mode == 2) 
                printf("Written value 0x%X to MMIO at offset 0x%lX, duration cycle: %ld\n", value, reg_offset, t2 - t1);
            else if (message_mode == 1)
                printf("Written 0x%X, TSC %ld\n", value, t2 - t1);
        }
    } else if (operation == 'r') {
        for(int i = 0; i < loop_count; i++){
            //reg_offset += i * 4;
            ...
            if (message_mode == 2) 
                printf("Read value 0x%X from MMIO at offset 0x%lX, duration cycle: %ld\n", value, reg_offset, t2 - t1);
            else if (message_mode == 1)
                printf("Read 0x%X, TSC %ld\n", value, t2 - t1);
        }
    } else if (operation == 'm') {
        value = 0x55AA55AA;
        for(int i = 0; i < loop_count; i++){
